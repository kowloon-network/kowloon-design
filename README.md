...
components/
  ...
  StateFeedback.md           — mostly resolved (Spinner/EmptyState/ErrorState); one open question (text register)
reference/
...
Every decision opened in `IDEOLOGY.md` §10 is resolved, and so are Button, Field, Heading/Eyebrow, Card/EventCard, Avatar, Reply, Modal/Sheet, TypeFilter, ReactButton, Toast, Timestamp, AddToCircle, and Notification. StateFeedback is mostly resolved — see `components/StateFeedback.md` for its one open question. `reference/style-guide.html` covers Button/Field/Heading; the rest aren't added yet.

- **`StateFeedback.md`**: should empty/error state text read as a small uppercase chrome label (web's current approach, consistent with Eyebrow/buttons/tags) or a plain conversational sentence (mobile's current approach, arguably better UX writing in the moment)? Not decided.
- Rewiring `@kowloon/client`, `kowloon-frontend`, and `kowloon-mobile` to actually consume this repo instead of the copy in `client` — **on hold until the component library is fully specced**, per Josh's instruction. Real implementation work is piling up behind that hold — see each component's own `.md` for its specific to-do list (token migrations, new features to build, dead code to delete, bugs to fix). It's substantial at this point; expect a real implementation project once the spec pass is done, not a quick find-and-replace. Toast in particular is a real feature build on mobile (a whole notification system, not a token tweak); StateFeedback is close behind at 62+24+38 files to migrate.
- The component library itself: Button, Field, Heading, Card, Avatar, Reply, Modal, TypeFilter, ReactButton, Toast, Timestamp, AddToCircle, and Notification are specced; StateFeedback is nearly there. Everything else in `kowloon-frontend/src/components` and `kowloon-mobile/src/components` still needs a contract — per `components/README.md`, check for *both* known failure modes (either platform can be the one with no shared component, or the one whose shared component has ballooned into an unmaintainable monolith), and actively look for real behavioral bugs/feature gaps/dead code, not just visual/structural drift.
