...
components/
  ...
  Notification.md            — mostly resolved; two open questions (icon color, unread indicator shape)
reference/
...
Every decision opened in `IDEOLOGY.md` §10 is resolved, and so are Button, Field, Heading/Eyebrow, Card/EventCard, Avatar, Reply, Modal/Sheet, TypeFilter, ReactButton, Toast, Timestamp, and AddToCircle. Notification is mostly resolved — see `components/Notification.md` for its two open questions. `reference/style-guide.html` covers Button/Field/Heading; the rest aren't added yet.

- **`Notification.md`**: (1) should notification-type icons be color-coded per type (web's current approach) or uniformly muted (mobile's)? (2) unread indicator — a small dot (web) or a left-edge bar (mobile)? Neither decided.
