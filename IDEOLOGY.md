...
1. **No rounded corners, anywhere — with one named exception.** Square corners on every surface: buttons, cards, inputs, images, modals. **User avatars are the one deliberate exception: circular, "person = circle" being a near-universal convention.** This wasn't a late carve-out — both platforms had already, independently, made the same call before this doc existed, with nearly identical reasoning in each codebase's own comments, which is a point in favor of it being right rather than drift. Circle/Group icons use a hexagon mask instead — their own deliberate brand mark, not a rounded-corner exception (a hexagon isn't "rounded"). No other exceptions: no "just this once for a pill badge."
