...
4. **Labels are uppercase and letter-spaced; body and content text is not.** Eyebrows, nav items, button labels, form field labels, bylines — all uppercase, wide tracking, small size. This is the single most consistent visual signature across every existing screen and it's not written down anywhere yet.
...
**Content roles (unaffected by anything below):** post/article body and titles use the serif reading face. Mobile's five-font, Kindle-style, user-selectable reading system ([[project_mobile_composer_editor]]-adjacent — see `kowloon-mobile/src/lib/typography.js`) is a real accessibility feature web doesn't have yet; that gap is noted, not something this pass needs to close.
