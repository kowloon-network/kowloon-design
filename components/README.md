...
[`Button.md`](Button.md), [`Field.md`](Field.md), [`Heading.md`](Heading.md), [`Card.md`](Card.md), [`Avatar.md`](Avatar.md), [`Reply.md`](Reply.md), [`Modal.md`](Modal.md), [`TypeFilter.md`](TypeFilter.md), [`ReactButton.md`](ReactButton.md), [`Toast.md`](Toast.md), [`Timestamp.md`](Timestamp.md), and [`AddToCircle.md`](AddToCircle.md) are all fully resolved. [`Notification.md`](Notification.md) is mostly resolved, two open questions (see below). The rest of the component library hasn't been speced yet; that's the actual work of the redesign pass, not something to backfill wholesale here.

**Notification is the third audit in a row to find dead code superseding an older component file** — same "found the file, but not the one actually rendering" trap as `ReplyList.jsx` and `FollowButton.jsx`'s chain. Worth naming as a standing check now: verify a web component file is actually imported by the real page before trusting it represents live behavior. Bigger finding: web still actively codes for `follow`-type notifications (icon, color, filter, routing) while mobile explicitly omits them, citing the add-to-circle-is-private policy by name in its own code comment — recommend removing web's, pending a server-side check that the type is genuinely never emitted. Going the other direction, web has a `moderation` notification type mobile has zero handling for — real gap to add. Two open judgment calls not resolved here: whether notification icons should be color-coded per type (web) or uniformly muted (mobile), and which unread-indicator shape (a dot vs. a left-edge bar) to standardize on.

**AddToCircle found dead code implementing the wrong product model.** `FollowButton.jsx` (literal "Follow"/"Unfollow" toggle) and its two consumers, `UserCard.jsx` and `UserList.jsx`, are all fully unreachable — nothing imports `UserList` anywhere. Leftover from before Kowloon dropped follow/unfollow for circles, never deleted. The real, live component (`AddToCircleButton.jsx`) is well-converged with mobile already, and its feedback-treatment gap folds directly into the Toast contract (mobile needs the same success/error toast web already has, including the "View circle" action link) rather than being a new decision. Also fed real behavior back into `Modal.md`: web's dialog here has ESC-to-close and scroll-lock that the shared `Modal` primitive itself lacks.
